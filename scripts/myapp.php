...
app.controller('dashboardCtrl', ['$scope','authFactory', '$location', '$http','$rootScope','dashAnalytics','userFactory',  function($scope, authFactory, $location, $http, $rootScope, dashAnalytics, userFactory){
  ...
  $scope.doPing = function(){
    $http.get("http://stg-api.foodtalkindia.com/user", {headers : {'Content-Type':'text/plain'}}).then(function(response){
...
app.factory('userFactory', ['$http','urlFactory', function($http, urlFactory){
  ...
  userFactory.getsearchresult = function(key, callback){
    $http.get("http://stg-api.foodtalkindia.com/api/search/user/"+key).then(function(response){
      callback(response);
    });
  }
  userFactory.getsearchtags = function(key, callback){
    $http.get("http://stg-api.foodtalkindia.com/api/search/user/tags/"+key).then(function(response){
      callback(response);
    });
  }
  return userFactory;
...
app.factory('eventFactory', ['$http','urlFactory', function($http, urlFactory){
  ...
  eventFactory.getEventDetails = function(id, callback){
    $http.get("http://stg-api.foodtalkindia.com/api/event/"+id+"/participants").then(function(response){
      callback(response);
    });
  }
  ...
  eventFactory.createNewEvent = function(name, s_date, loc, cost, timings, venue, cover_url, payment_url, description, callback){
    console.log('factory enter');
      $http({
          method: 'POST',
          url: 'http://stg-api.foodtalkindia.com/api/event',
  ...
  eventFactory.editEvent = function(id, name, s_date, loc, cost, timings, venue, cover_url, payment_url, description, callback){
    console.log('factory enter');
      $http({
          method: 'PUT',
          url: 'http://stg-api.foodtalkindia.com/api/event/'+id,
          data : {name:name, start_date:s_date, location:loc, cost:cost, timings:timings, venue:venue, cover_url:cover_url, payment_url:payment_url, description:description}
        }).then(function (response) {
            if(response.data.message === "Success"){
                callback(true);
...
app.factory('urlFactory', [ function(){
  ...
  url.dashboard = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/dashboard/batcave";
  url.login= "<?php echo 'http://stg-api.foodtalk.in'; ?>/login";
  url.logout = "<?php echo 'http://stg-api.foodtalk.in'; ?>/logout";
  url.users = "<?php echo 'http://stg-api.foodtalk.in'; ?>/user";
  url.contest = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/contest";
  url.events = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/event";
  url.vendors = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/vendor";
  url.influencer = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/influencer";
  url.bloggers = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/bloggers";
  url.media = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/media";
  url.vendorscat = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/vendor/category";
  url.influencercat = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/influencer/category";
  url.bloggerscat = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/bloggers/category";
  url.mediacat = "<?php echo 'http://stg-api.foodtalk.in'; ?>/api/media/category";
  return url;
}]);
